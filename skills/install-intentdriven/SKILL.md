---
name: install-intentdriven
description: Use when installing, reinstalling, or verifying the local intentdriven plugin in Pi, OMP, Claude Code, or Codex after its skills or manifests change.
---

# Install IntentDriven
...
This skill owns only `intentdriven` installation. It does not publish the repository, install missing harnesses, alter unrelated plugins, or replace a remote marketplace unless the user authorizes the local-source switch.
...
Locate this skill's owning repository and resolve its absolute path as `root`; do not assume the current working directory is the plugin checkout. Confirm the package and plugin names are `intentdriven` and read all five versions:
...
| OMP | `omp plugin list --json`; inspect the `intentdriven` marketplace source |
...
Preserve an intentionally remote marketplace unless the user explicitly requested this local checkout as its replacement. Refresh or remove only the `intentdriven` registration; never clear shared caches or unrelated marketplaces.
...
When `.agents/plugins/marketplace.json` exists, confirm its authentication policy is a supported value such as `ON_INSTALL` or `ON_USE`; an obsolete policy blocks Codex marketplace registration.
...
| OMP | `omp plugin marketplace add "$root"`; `omp plugin install intentdriven@intentdriven --force`; `omp plugin list --json`; `omp plugin discover intentdriven` | Local source, intended version, and expected skill inventory. |
| Claude Code | `claude plugin marketplace add "$root" --scope user`; `claude plugin install intentdriven@intentdriven --scope user -y --json`; `claude plugin list --json` | Intended plugin ID/version and local marketplace source. |
| Codex | `codex plugin marketplace add "$root" --json`; `codex plugin add intentdriven@intentdriven --json`; `codex plugin list --available --json` | Intended plugin ID/version enabled through the local marketplace. |
...
| "The installed version already matches." | Compare the installed installer skill with the checkout. If the same-version cache is stale, use that harness's uninstall/install flow for only `intentdriven`; never delete shared caches. |
| "The README command is probably current." | Read current CLI help when command shape is uncertain. |
| "The local marketplace means a local plugin payload." | Verify the plugin source is `./`; a URL source still installs remotely. |
...
- A local marketplace manifest points the plugin payload at a remote URL.
...
Do not call the local installation complete until the five versions agree, every available native check has an observed result, every target has one allowed status with evidence, each installed target shows the intended local source and version, cached installer content matches the checkout, and activation is reported separately.
