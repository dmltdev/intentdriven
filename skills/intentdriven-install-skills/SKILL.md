...
Preserve an intentionally remote registration unless the user requested this local checkout as its replacement. Refresh or remove only the `intentdriven` registration.
...
| Claude Code | Refresh only the `intentdriven` marketplace from `root`; run `claude plugin install intentdriven@intentdriven --scope user -y --json`, then `claude plugin list --json`. | The intended plugin ID, version, and local marketplace source are visible. |
...
Compare the installed installer skill with the checkout. If a same-version cache is stale, use that harness's uninstall/install flow for only `intentdriven`; never delete shared caches.
...
|---|---|
| "The install command exited zero." | Inspect the post-install source, version, skill inventory, and installer content before using `installed`. |
...
Do not report completion unless all five manifest versions agree, every available native check has an observed result, every harness has a separate status with observed evidence or an exact skip/blocker, cached installer content matches the checkout, and activation is reported separately.
